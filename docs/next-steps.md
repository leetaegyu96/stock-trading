...
- **장중 자동매매(인트라데이) 구현 완료 — 진행 중(운영 미활성화)**: `feature/intraday-trading-loop` 브랜치에서 9태스크(신규 지표·config·DecisionType·엔진 게이트/체결·KIS 체결강도·오케스트레이터 잠정봉·스케줄러·대시보드 라벨/문서) 전부 구현·테스트 통과(trading-rules §17). `intraday_enabled` 기본 `False`라 아직 미동작. 다음: 전체브랜치 리뷰(opus) → dev 병합 → 운영 서버 플래그 `True` 활성화 → 첫 장중 실동작(매수/매도·킬스위치) 로그 관측 → 이상 없으면 v1.10.0에 포함해 릴리즈.
...
- [ ] **v1.10.0 릴리즈** — dev→main 승격, 태그, 패치노트(PR #10~13 묶음, 인트라데이 포함 여부는 활성화 검증 결과에 따라 결정).
...
- [ ] **장중 자동매매 운영 활성화** — `intraday_enabled=True`로 별도 설정/환경변수 켜기 → 장중 로그로 매수/매도·킬스위치 실동작 첫 관측 → KIS 체결강도(`cttr`) 필드가 실서버 응답과 일치하는지 검증(trading-rules §17, 문서상 값이라 미검증) → 이상 없으면 누적분 v1.10.0 릴리즈.
