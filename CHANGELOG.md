...
### 결과
- 장중 ON 시 국내형 −17.98%p 의 분해: **비용 +0.59%p(전체의 3.3%)**, **매수 프리미엄은 오히려 개선**(+0.51%→+0.46%, "고점 매수" 가설 기각), **실현손익 −15.81%p**(거의 전부)
- 원인은 **회전 그 자체** — 평가 기회가 8배로 늘어 매수가 자주 성립하고, 그 포지션이 같은 날 저가에 노출돼 손절·트레일에 더 걸린다. 점수 매도가 꺼진 상태에서도 나빠진다

### Changed
- `docs/trading-rules.md` §17-1 우선순위 정정 — 거래량 배율 의존 신호는 점수 매도에만 쓰이므로, **시간대별 거래량 프로파일을 고쳐도 −17.6~−18.0%p 는 그대로**다. 재활성화 선결 과제는 **회전 억제**(진입 후 유예 / 일일 매수 상한 / 장중 매도-전용 비대칭)
